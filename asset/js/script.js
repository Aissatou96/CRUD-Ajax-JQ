$(function(){
    ...
    const getDonneesUser = (id=0) =>{
        $.ajax({
            method:"GET",
            url: `./src/page/getUsers.php?id=${id}&nb_elt=${nb}&page=${page}`
        })
        .done(data => {
...
// -----------------------------------------------------------
    ...
    const objUser=(datas) => {
        ...
        for(const data of datas){
            const {email,cell,nat,picture,location:{coordinates},dob:{date},name:{title,first,last}}=data;
